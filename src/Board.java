...
public class Board implements Cloneable {
	...
	public SolveStatus seek() {
		...
		try {
			...
			while (changed == true) {
				...
				for (int i = 0; i <= highestNumber; i++) {
					for (int j = i; j <= highestNumber; j++) {
						if (potentialPositionsForTiles[i][j].hasExactlyOne()
								&& potentialPositionsForTiles[i][j].done == false) {
							potentialPositionsForTiles[i][j].done = true;
							changed = true;
							Position position = potentialPositionsForTiles[i][j]
									.getFirst();
			...
			for (int r = 0; r < numRows; r++) {
				for (int c = 0; c < numColumns; c++) {
					...
					if (potDir.done == false
							&& potDir.hasOnlyOneDirection() == true) {
						potDir.done = true;
						changed = true;
						...
						setUniquePosition(thisNumber, otherNumber,
								new Position(top, left, dir));
		...
		if (solveStatus == SolveStatus.INCOMPLETE) {
			...
			if (solveStatus == SolveStatus.INCOMPLETE) {
				System.out
						.println("This shouldn't happen, after the recursion the status should either be solved or conflict, not incomplete.");
			}
			if (solveStatus == SolveStatus.SOLVED) {
				// accept the guess
				applyState(newBoard);
	...
	public void markGuessImpossible(Guess guess) {
		...
		try {
			potentialPositionsForTiles[position.getLowerNumber()][position
					.getHigherNumber()].remove(position);
		...
		try {
			if (position.orientation == Orientation.HORIZONTAL) {
				...
				potentialDirections[position.secondRow()][position
						.secondColumn()].clearLeft();
			} else if (position.orientation == Orientation.VERTICAL) {
				potentialDirections[position.row][position.column].clearDown();
				potentialDirections[position.secondRow()][position
						.secondColumn()].clearUp();
			}
		} catch (UnsolvableException ex) {
			System.out
					.println("This shouldn't happen, when we mark a guess as impossible there can't be zero directions left in this location");
		}
	}

	public static Board createInitialBoard(int highestNumber, int numRows,
			int numColumns, int[][] numbersOnBoard) {
																	...
																	// one
			{
				...
				if (numberOnLeftTile <= numberOnRightTile)
					potentialPositionsForTiles[numberOnLeftTile][numberOnRightTile]
							.add(new Position(i, j, Orientation.HORIZONTAL));
				else
			...
			for (int j = 0; j < numbersOnBoard[i].length; j++) // all columns
			{
				...
				if (numberOnTopTile <= numberOnBottomTile)
					potentialPositionsForTiles[numberOnTopTile][numberOnBottomTile]
							.add(new Position(i, j, Orientation.VERTICAL));
				else
	...
	private void removeHorizontalTile(Position position)
			throws UnsolvableException {
	...
	public void removeVerticalTile(Position position)
			throws UnsolvableException {
		int r = position.row, c = position.column;
	 ...
	 * @throws UnsolvableException
	 */
	public void setUniquePosition(int firstNumber, int secondNumber,
			Position uniquePosition) throws UnsolvableException {
		...
		LinkedList<Position> removedPositions = potentialPositionsForTiles[smallerNumber][biggerNumber]
				.setUniqueCoordinate(uniquePosition);
		...
		Iterable<Position> overlappingPositions = uniquePosition
				.getOverlappingPositions();
		for (Position overlapPos : overlappingPositions) {
			potentialPositionsForTiles[overlapPos.getLowerNumber()][overlapPos
					.getHigherNumber()].remove(overlapPos);
		}
	}
